...
pub fn prepare_backbuffer(buffer: &mut BackBuffer, draw_size: &[u32; 2], zoom_level: f64, aircraft: &AircraftData) {
    clear_backbuffer(buffer);

    // Render aircraft
    let aircraft_rendered = aircraft.data.iter()
        .map(|x| render_aircraft(x, buffer, draw_size, zoom_level))
        .filter(|&x| x)
        .count();
...
pub fn perform_rendering(g: &mut G2d, context: &Context, render_size: (f64, f64), zoom_level: f64, geo_data: &GeoData) {
    piston_window::clear([0.0, 0.0, 0.0, 1.0], g);

    // Render geography
    let geo_segments_rendered = geo_data.coast
        .iter()
        .map(|x| render_coastline(x, g, context, render_size, zoom_level))
        .sum::<usize>();
...
fn render_aircraft(aircraft: &Aircraft, buffer: &mut BackBuffer, view_size: &[u32; 2], zoom_level: f64) -> bool {
    if let (Some(lon), Some(lat)) = (aircraft.longitude, aircraft.latitude) {
        ...
        let (x_norm_scaled, y_norm_scaled) = (x_norm * zoom_level, y_norm * zoom_level);
...
fn render_coastline(data: &CoastlineDataEntry, g: &mut piston_window::G2d, context: &Context, render_size: (f64, f64), zoom_level: f64) -> usize {
    let transformed = data.vertices.iter()
        .map(|v| coords::normalised_equirectangular_coords(v[0], v[1]))
        .map(|v| [v.0 * zoom_level, v.1 * zoom_level])
        .collect::<Vec<[f64; 2]>>();
